...
def render_decision_card(
    ...
    with st.container(border=True):
        ...
        with header_col1:
            st.markdown(
                f"**{ticker}** &nbsp; {render_action_badge(action)}",
        ...
        # Rationale (in expander to save space)
...
def render_thesis_card(
    ...
    with st.container(border=True):
        ...
        with header_col1:
            st.markdown(
                f"**{ticker}** &nbsp; {render_status_badge(status)}",
                unsafe_allow_html=True,
            )
            if ai_generated:
                st.caption(f"AI: {thesis.get('ai_model', 'unknown')}")

        with header_col2:
            st.markdown(thesis.get("summary", "No summary")[:120] + ("..." if len(thesis.get("summary", "")) > 120 else ""))
